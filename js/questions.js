...
export function openQForm(id = null){
  ...
  if(id){
    ...
    ['qf-text','qf-image','qf-a','qf-b','qf-c','qf-d'].forEach(id => {
      if($(id)) $(id).value = '';
    });
...
export async function saveQ(){
  ...
  const a = $('qf-a').value.trim();
  const b = $('qf-b').value.trim();
  const c = $('qf-c').value.trim();
  const d = $('qf-d').value.trim();

  if(!text || !a || !b || !c || !d){
    alert('Vui lòng điền đầy đủ nội dung câu hỏi và 4 đáp án!');
    return;
  }
...
export function renderQuestions(){
  ...
  if(filterSC){
    qs = qs.filter(q => q.subcat === filterSC);
  } else if(filterCat){
    qs = qs.filter(q => q.cat === filterCat);
  }

  const groups = [...new Set(
    qs.map(q => q.subcat || q.cat || '(Chưa phân loại)')
  )].sort();

  $('q-list').innerHTML = groups.map(sc => {
    const sub = qs.filter(q => 
      (q.subcat || q.cat || '(Chưa phân loại)') === sc
    );

    return `
      <div class="cat-hdr">${esc(sc)} (${sub.length})</div>
      ${
        sub.map(q => `
          <div class="qitem">
            <div class="qrow">
              <div class="qtext">
                ...
                ${
                  q.image 
                    ? `<img class="q-img" src="${esc(q.image)}" alt="Hình ảnh câu hỏi">` 
                    : ''
                }
              </div>
